        ...
        for edge in edges:
            if current == edge[0]:
                for newstate in edges[edge]:
                    path = nfsmaccepts(newstate, edges, accepting, newvisited)
                    if path != None:
                        return edge[1] + path
        return None  
...
def nfsmtrim(edges, accepting): 
    states = []
    for n in edges:
        states = states + [n[0]] + edges[n]
    live_states = []    
    for s in states:    
        if nfsmaccepts(s, edges, accepting,[]) != None:
            live_states = live_states + [s]
    # all live states
    new_edges = {}
    new_accepting = []
    for n in edges:
        if n[0] in live_states:
            goto = []
            for newstate in edges[n]:
                if newstate in live_states:
                    goto = goto + [newstate]
            if goto != []:
                new_edges[n] = goto
    for s in accepting:
