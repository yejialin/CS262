...
# expend the token which can rewrite in grammar
def expend(tokens_and_derivation, grammar):
    (tokens, derivation) = tokens_and_derivation
    for tok_pos in range(len(tokens)):
        for rule_idx in range(len(grammar)):
            rule = grammar[rule_idx]
            if tokens[tok_pos] == rule[0]:
                yield ((tokens[0:tok_pos]+rule[1]+tokens[tok_pos+1:]),derivation+[rule_idx])          
                                
def isambig(grammar, start, utterance):
    enumerated = [([start],[])]
    while True:                                              #while means many time
        new_enumerated = enumerated
        for u in enumerated:
            for i in expend(u,grammar):                      #expand one degree
                if not i in new_enumerated:
                    new_enumerated = new_enumerated + [i]
        if new_enumerated != enumerated:                     #update
