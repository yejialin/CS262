...
#tu see if Q can be rewritten to xQy and sizexy > 0 
def helpfunction(grammar, current, visited, sizexy):
    if current in visited:
        return sizexy > 0
    else:
        new_visited = visited + [current]
        for rhs in [r[1] for r in grammar if current == r[0]]:
            for s in rhs:
                if helpfunction(grammar, s, new_visited, len(rhs) - 1):
                    return True
        return False
    
def cfginfinite(grammar): 
    for Q in[r[0] for r in grammar]:
        if helpfunction(grammar, Q, [], 0):
            return True
    return False
