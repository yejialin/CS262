...
def cfgempty(grammar, symbol, visited):
    if symbol in visited:
        return None
# is symbol a terminal    
    elif not any(symbol == rule[0] for rule in grammar):
        return [symbol]
    else:
        new_visited = visited + [symbol]
        for rs in [r[1] for r in grammar if r[0] == symbol]:
            if all(None != cfgempty(grammar, r, new_visited) for r in rs):
                result = []
                for r in rs:
